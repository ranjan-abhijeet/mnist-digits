...
"""
2nd method of building deeplearning solution is using a function
which returns the model
"""
def model_function():
    input_layer = Input(shape=(28,28,1))
    model = Conv2D(28, (3,3), activation='relu')(input_layer)
    model = Conv2D(56, (3,3), activation='relu')(model)
    model = MaxPool2D()(model)
    model = BatchNormalization()(model)
    model = Conv2D(112, (3,3), activation='relu')(model)
    model = MaxPool2D()(model)
    model = BatchNormalization()(model)
    model = GlobalAvgPool2D()(model)
    model = Dense(56, activation='relu')(model)
    model = Dense(10, activation='softmax')(model)
    
    final_model = tf.keras.Model(inputs=input_layer, outputs=model)
    return final_model
